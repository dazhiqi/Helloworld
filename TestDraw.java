...
class MyBoom {
	...
	public MyBoom(float x, float y, float degree) {
		...
		scale = 0.2f;
		try {
			data = ImageIO.read(new File("src/Game/boom.png"));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
...
class MyBullet {
	...
	public MyBullet(float x, float y, float speedX, float speedY, float degree) {
		...
		this.degree = degree;
		try {
			data = ImageIO.read(new File("src/Game/bullet.png"));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
...
class MyEnemy {
	...
	public MyEnemy(float x, float y) {
		...
		speed = 1;
		try {
			data = ImageIO.read(new File("src/Game/rubbish.png"));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
...
class MyCharactor {
	...
	public MyCharactor(float x, float y) {
		...
		speed = 2;
		try {
			data = ImageIO.read(new File("src/Game/player.png"));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
...
class MyPanel extends JPanel implements MouseMotionListener {
	...
	public MyPanel() {
		...
		try {
			back = ImageIO.read(new File("src/Game/background.jpg"));
